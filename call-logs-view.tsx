src\components\custom\call-logs-view.tsx
...
export function CallLogsView({ teamId, memberId }: CallLogsViewProps) {
  ...
  const isDateInRange = (dateStr: string) => {
    ...
    // Parse the date string into a Date object, preserving timezone
    const date = new Date(dateStr);
    
    // Create start date at beginning of day (00:00:00)
    const start = new Date(dateRange.start);
    start.setFullYear(date.getFullYear()); // Set to same year as the log date
    start.setHours(0, 0, 0, 0);
    
    // Create end date at end of day (23:59:59.999)
    const end = new Date(dateRange.end);
    end.setFullYear(date.getFullYear()); // Set to same year as the log date
...
/>
  ...
  <div className="flex-col gap-1" style={{ height: 'calc(100% - 20px)' }}>
  ...
  <DialogContent className="!p-0 !m-0 border bg-white overflow-hidden max-w-[600px] fixed top-[37%] left-[50%] transform -translate-x-1/2 -translate-y-1/2] z-50">
    <Calendar 
      onSelectRange={(start, end, quickSelect) => {
        if (quickSelect) {
          const now = new Date();
